...
if __name__ == "__main__":
    ...
    ytr=np.int_(ytr)
    ytr_ohe = np.zeros((ytr.size, ytr.max()+1))
    ytr_ohe[np.arange(ytr.size),ytr] = 1
    wn.set_training_data(dataset_name, Xtr, ytr_ohe)
    ...
    # Obtain the normalizer and apply to the data
    normalizer = wn.get_normalizer()
    ...
    y = np.argmax(ytst_ohe, axis=-1) # Convert to labels
    classes = np.arange(ytst_ohe.shape[1]) 
