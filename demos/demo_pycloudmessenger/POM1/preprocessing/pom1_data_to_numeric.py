...
December 2020
python pom1_data_to_numeric.py --user <user> --password <password> --task_name <task_name> --dataset <dataset>
...
if __name__ == "__main__":
    ...
    display('------- Sample before ---------------------------------', logger, verbose)
    display(Xval[0], logger, verbose)

    Xval = data2num_transformer.transform(Xval)

    display('------- Sample after ---------------------------------', logger, verbose)
    display(Xval[0, :], logger, verbose)

    display('----------------------------------------------------------------------', logger, verbose)
    display('------------------------- Data to numeric completed ------------------', logger, verbose)
    display('----------------------------------------------------------------------\n', logger, verbose)
