...
December 2020
python pom1_check_data.py --user <user> --password <password> --task_name <task_name> --dataset <dataset>
...
if __name__ == "__main__":
    ...
    display('Checking data at workers', logger, verbose)
    err, bad_workers = mn.check_data_at_workers(input_data_description, target_data_description)
    if err is not None:
        display('Bad workers: %s' %bad_workers, logger, verbose)
        display(err, logger, verbose)

    # At this point two options are possible: 
    #    - we disconnect the bad workers, and continue with the training process
    #    - we stop the whole process

    disconnect_bad_workers = True
    if disconnect_bad_workers:
        mn.terminate_workers(bad_workers)
        # Resetting error to continue with training (optional)
        # err = None
    ...
    display('-------------------------------------------------------------------------', logger, verbose)
    display('-------------------- Check data at workers completed --------------------', logger, verbose)
    display('-------------------------------------------------------------------------\n', logger, verbose)
