...
if __name__ == "__main__":
    ...
    display('------- Sample before normalization  -----------------\n', logger, True)
    display(Xval[0, :], logger, True)

    ##############################################################################
    ######## Data normalization prior to PCA  ################################
    # normalizes workers data and returns the normalizer object
    normalizer = mn.normalizer_fit_transform_workers(new_input_data_description, 'global_mean_std', 'all') 
    #normalizer = mn.normalizer_fit_transform_workers(new_input_data_description_fs, 'global_min_max') 
    # Normalizing Val data
    Xval = normalizer.transform(Xval)

    display('------- Sample after normalization  ---------------------\n', logger, True)
    display(Xval[0, :], logger, True)
    ...
    display('\nError in computing mean = %f ' % np.linalg.norm(global_mean - centralized_mean), logger, True)
    display('\nError in computing std = %f ' % np.linalg.norm(global_std - centralized_std), logger, True)

    display('\n---------------------------------------------------------------------', logger, True)
    display('------------------------- Data normalization completed ----------------', logger, True)
    display('----------------------------------------------------------------------\n', logger, True)
