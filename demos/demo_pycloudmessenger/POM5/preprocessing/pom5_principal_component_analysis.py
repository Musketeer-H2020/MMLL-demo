...
if __name__ == "__main__":
    ...
    display('------- Sample before PCA  -----------------\n', logger, True)
    display(Xval[0, :], logger, True)
    
    pca_transformer, new_input_data_description, workers_errors = mn.pca_fit_transform_workers(new_input_data_description, 'direct', NF)
    Xval = pca_transformer.transform(Xval)

    display('------- Sample after PCA  -----------------\n', logger, True)
    display(Xval[0, :], logger, True)

    display('\n---------------------------------------------------------------------', logger, True)
    display('----------------------- PCA completed -------------------------------', logger, True)
    display('----------------------------------------------------------------------\n', logger, True)
