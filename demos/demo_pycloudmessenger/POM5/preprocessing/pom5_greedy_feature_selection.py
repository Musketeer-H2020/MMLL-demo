...
if __name__ == "__main__":
    ...
    [transformer, new_input_data_description, errors_data2num] = mn.data2num_transform_workers(input_data_description) 
    [Xval, yval] = dc.get_data_val()
    if transformer is not None:
        Xval = transformer.transform(Xval)

    # direct
    #[ranked_inputs, performance_evolution] = mn.rank_features_gfs(Xval, yval, new_input_data_description, method='direct')
    #[ranked_inputs, performance_evolution] = mn.rank_features_gfs(Xval, yval, new_input_data_description, method='direct', NF=30)
    #[ranked_inputs, performance_evolution] = mn.rank_features_gfs(Xval, yval, new_input_data_description, method='direct', stop_incr=0.01)
    # roundrobin
    #[ranked_inputs, performance_evolution] = mn.rank_features_gfs(Xval, yval, new_input_data_description, method='roundrobin')
    #[ranked_inputs, performance_evolution] = mn.rank_features_gfs(Xval, yval, new_input_data_description, method='roundrobin', NF=30)
    #[ranked_inputs, performance_evolution] = mn.rank_features_gfs(Xval, yval, new_input_data_description, method='roundrobin', stop_incr=0.01)
    ...
    [ranked_inputs, performance_evolution] = mn.rank_features_gfs(Xval, yval, new_input_data_description, method='direct', NF=NF)
    

    display('------- Sample before Greedy Feature Selection  -----------------\n', logger, True)
    display(Xval[0, :], logger, True)

    from MMLL.preprocessors.feature_extract import feature_extract_model
    feature_extractor = feature_extract_model(ranked_inputs, new_input_data_description)
    Xval = feature_extractor.transform(Xval)
    new_input_data_description_fs = feature_extractor.new_input_data_description
    display('Preprocessing data at workers with feature_extract', logger, True)
    errors_feature_extract = mn.preprocess_data_at_workers(feature_extractor)

    display('------- Sample after Greedy Feature Selection  -----------------\n', logger, True)
    display(Xval[0, :], logger, True)

    display('\n---------------------------------------------------------------------', logger, True)
    display('----------------------- Greedy Feature Selection completed ------------', logger, True)
    display('----------------------------------------------------------------------\n', logger, True)
