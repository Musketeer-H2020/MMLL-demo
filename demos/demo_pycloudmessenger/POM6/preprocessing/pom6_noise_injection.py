...
Nov. 2020
python3 pom6_noise_injection.py --dataset pima_raw --verbose 1
...
if __name__ == "__main__":
    ...
    display('\n---------------------------------------------------------------------', logger, True)
    display('------------------------- Data to numeric completed  --------', logger, True)
    display('----------------------------------------------------------------------\n', logger, True)


    display('\n---------------------------------------------------------------------', logger, True)
    display('------------------------- Noise injection    --------', logger, True)
    display('----------------------------------------------------------------------\n', logger, True)
    from MMLL.preprocessors.noise_injection import noise_injection_model
    prep_noise_injection = noise_injection_model(new_input_data_description, alfa = 0.2)

    display('Preprocessing data at workers with noise_injection...', logger, True)
    errors_noise_injection = mn.preprocess_data_at_workers(prep_noise_injection)
    display('Preprocessing data at workers with noise_injection is DONE', logger, True)
    print('Errors = ', errors_noise_injection)
