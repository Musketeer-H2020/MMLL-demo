...
Example of use: python pom2_NN_master_pycloudmessenger.py --user <user> --password <password> --task_name <task_name>
...
if __name__ == "__main__":
    ...
    try:
        with open('./keras_model_MLP_light.json', 'r') as json_file:
            model_architecture = json_file.read()
    except:
        display('Error - The file keras_model_MLP_light.json defining the neural network architecture is not available, please put it under the following path: "' + os.path.abspath(os.path.join("","./")) + '"', logger, verbose)
        sys.exit()
    ...
    #########################################
    display('Creating MasterNode under POM2, communicating through pycloudmessenger', logger, verbose)
