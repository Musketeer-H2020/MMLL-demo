...
December 2020
python pom2_missing_data_imputation.py --user <user> --password <password> --task_name <task_name> --dataset <dataset>
...
if __name__ == "__main__":
    ...
    if dataset_name in ["income_raw_V_missing", "income_raw_V"]:
    ...
    linkage_type = 'full'  # Only full records are considered
    linkage_type = 'join' # All data is joined, but missing values can be present

    input_data_description_dict, target_data_description_dict = mn.record_linkage_transform_workers(linkage_type=linkage_type)


    # Transform data to numerical for vertical partitioning
    ...
    # Missing values in training data, Mean IMPUTATION
    from MMLL.preprocessors.data_imputation import imputation_model_V
    imputator = imputation_model_V(new_input_data_description)
    errors_imputation = mn.preprocess_data_at_workers_V(imputator)

    display('---------------------------------------------------------------------', logger, verbose)
    display('------------- Missing data imputation completed  --------------------', logger, verbose)
    display('---------------------------------------------------------------------\n', logger, verbose)
